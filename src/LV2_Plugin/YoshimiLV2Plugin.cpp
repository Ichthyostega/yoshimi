...
void YoshimiLV2Plugin::process(uint32_t sample_count)
{
     ...
     */
/////////TODO buffer branch(Ichthyo 8/2019): not clear yet if the above mentioned problems are completely resolved through the
/////////TODO                              : changes in the processing pattern (pull processing) introduced with the "buffer branch".
/////////TODO                              : As it seems, all systems are behaving consistently now -- however, using a large *processing buffer*
/////////TODO                              : and thus a low k-Rate sometime leads to very noticeable sonic changes; the special cases investigated
/////////TODO                              : in detail all turned out to be caused by the nature of the algorithm and thus can not be fixed.
/////////TODO                              : Most notably: patterns in aliased overtones, and the exponential damping of parameters
...
void YoshimiLV2Plugin::connect_port(LV2_Handle instance, uint32_t port, void *data_location)
{
    ...
    {
        inst->synth->getRuntime().Log("LV2-Plugin: invalid output port number encountered: port=" + asString(port));
        return;
    }
