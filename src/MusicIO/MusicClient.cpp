...
{
    unique_ptr<BeatTracker> beat;
    if (useAudio == jack_audio && useMidi == jack_midi)
        beat = make_unique<SinglethreadedBeatTracker>();
    else
        beat = make_unique<MultithreadedBeatTracker>();

    switch(useAudio)
    {
        case jack_audio:
            audioIO = make_shared<JackEngine>(synth, move(beat));
            break;
#if defined(HAVE_ALSA)
        case alsa_audio:
            audioIO = make_shared<AlsaEngine>(synth, move(beat));
            break;
#endif
        default:
            break;
    ...
    switch(useMidi)
    {
        case jack_midi:
            if (useAudio == jack_audio)
                ...
                midiIO = make_shared<JackEngine>(synth, move(beat));
            break;
#if defined(HAVE_ALSA)
        case alsa_midi:
            if (useAudio == alsa_audio)
                ...
                midiIO = make_shared<AlsaEngine>(synth, move(beat));
            break;
#endif
        default:
            break;
